...
namespace TestHelpers.CodeGenerator
{
    public static class FactoryMethodGenerator
    {
        ...
        {
            var type = typeof(T);
            return CreateForType(type, outputAction);
        }

        public static string CreateForType(Type type, Action<string> outputAction = null)
        {
        ...
        internal class FactoryMethodCodeBuilder
        {
            ...
            public void HandleProperty(PropertyInfo property)
            {
                ...
                else if (propertyType.IsDecimalNumberType())
                {
                    ...
                    HandleProperty(
                        property,
                        numericTypeInfo.Alias,
                        GetNextDecimalNumber(numericTypeInfo.Suffix));
                }
            ...
            private void HandleGenericProperty(PropertyInfo property, Type propertyType)
            {
                ...
                if (propertyType.IsDictionary())
                {
                    HandleProperty(
                        ...
                        genericType,
                        "null",
                        $" ?? new Dictionary{genericArgumentList}()");
                }
                ...
                string localVariable = "")
            {
                ...
                AddAssignment(
                    ...
                    parameterName,
                    extraAssignment);
                if (!string.IsNullOrWhiteSpace(localVariable))
                {
